...
func TestRemoteControlPlaneTs(t *testing.T) {
	rt.ResourceTest(t, "remote/control-plane-ts", getJSBaseOptions(t), func(ctx *rt.ResourceContext) {
		rt.Validate(ctx, "kubernetes-the-hard-way:remote:ControlPlaneNode", "simple", func(t *testing.T, res apitype.ResourceV3) {
			...
			rt.ExpectOutput(t, res, "caPrivateKeyPath", "/tmp/caKey")
			...
			rt.ExpectOutput(t, res, "kubeApiServerCertificatePath", "/tmp/kubeApiPem")
			rt.ExpectOutput(t, res, "kubeApiServerInstallDirectory", "/usr/local/bin")
			rt.ExpectOutput(t, res, "kubeApiServerPrivateKeyPath", "/tmp/kubeApiKey")
			rt.ExpectOutput(t, res, "kubeControllerManagerInstallDirectory", "/usr/local/bin")
			rt.ExpectOutput(t, res, "kubeControllerManagerKubeconfigPath", "/tmp/kubeControllerManagerKubeconfig")
			...
			rt.ExpectOutput(t, res, "kubeSchedulerKubeconfigPath", "/tmp/kubeSchedulerKubeconfig")
			rt.ExpectOutput(t, res, "nodeName", "server")
			rt.ExpectOutput(t, res, "serviceAccountsCertificatePath", "/tmp/saPem")
			rt.ExpectOutput(t, res, "serviceAccountsPrivateKeyPath", "/tmp/saKey")
