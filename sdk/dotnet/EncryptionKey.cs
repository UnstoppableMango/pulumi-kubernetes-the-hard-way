// *** WARNING: this file was generated by pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace UnMango.KubernetesTheHardWay
{
    [KubernetesTheHardWayResourceType("kubernetes-the-hard-way:index:EncryptionKey")]
    public partial class EncryptionKey : global::Pulumi.ComponentResource
    {
        /// <summary>
        /// The generated `v1/EncryptionConfig`.
        /// </summary>
        [Output("config")]
        public Output<string> Config { get; private set; } = null!;

        [Output("key")]
        public Output<Pulumi.Random.RandomBytes> Key { get; private set; } = null!;


        /// <summary>
        /// Create a EncryptionKey resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public EncryptionKey(string name, EncryptionKeyArgs? args = null, ComponentResourceOptions? options = null)
            : base("kubernetes-the-hard-way:index:EncryptionKey", name, args ?? new EncryptionKeyArgs(), MakeResourceOptions(options, ""), remote: true)
        {
        }

        private static ComponentResourceOptions MakeResourceOptions(ComponentResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new ComponentResourceOptions
            {
                Version = Utilities.Version,
                PluginDownloadURL = "github://api.github.com/UnstoppableMango",
            };
            var merged = ComponentResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
    }

    public sealed class EncryptionKeyArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The length of the key in bytes.
        /// </summary>
        [Input("bytes")]
        public Input<double>? Bytes { get; set; }

        public EncryptionKeyArgs()
        {
        }
        public static new EncryptionKeyArgs Empty => new EncryptionKeyArgs();
    }
}
