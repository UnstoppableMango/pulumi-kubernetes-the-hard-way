...
func NewMkdir(ctx *pulumi.Context,
	...
	var resource Mkdir
	err := ctx.RegisterResource("kubernetes-the-hard-way:tools:Mkdir", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetMkdir(ctx *pulumi.Context,
	...
	var resource Mkdir
	err := ctx.ReadResource("kubernetes-the-hard-way:tools:Mkdir", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
type MkdirArgs struct {
	...
	// The command to run on create.
	Create MkdirOptsPtrInput
	// The command to run on delete. The environment variables PULUMI_COMMAND_STDOUT
	// and PULUMI_COMMAND_STDERR are set to the stdout and stderr properties of the
	// Command resource from previous create or update steps.
	Delete MkdirOptsPtrInput
	// Environment variables
